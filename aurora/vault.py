...
class VaultClient:
    ...
    def get_token_info(self, clientid):
            ...
            for accesor in accesor_list:
                ...
                if r_accesor_query.status_code == 200:
                    if r_accesor_query.json()['data']['meta'] is not None:
                        if r_accesor_query.json()['data']['meta']['clientid'] == clientid:
                            already_exist = True
    ...
    def create_token(self, clientid):
        r = self.api('/auth/token/accessors', 'LIST')
        if r.status_code == 200:
            accesor_list = r.json()['data']['keys']
            already_exist = False
            ...
            if not already_exist:
                payload = {"policies": ["policy-" + clientid],
    ...
    def create_policy(self, clientid):
        r = self.api('/sys/policy', 'GET')
        if r.status_code == 200:
            keys = r.json()['data']['policies']
            if 'policy-' + clientid not in keys:
    ...
    def delete_token(self, clientid):
        r = self.api('/auth/token/accessors', 'LIST')
        if r.status_code == 200:
            accesor_list = r.json()['data']['keys']
            already_exist = False
    ...
    def delete_policy(self, clientid):
        r = self.api('/sys/policy', 'GET')
        if r.status_code == 200:
            policies = r.json()['data']['policies']
            if 'policy-' + clientid in policies:
    ...
    def decrypt(self, clientid, source_filename):
        r = self.api('/transit/keys', 'LIST')
        if r.status_code == 200:
            keys = r.json()['data']['keys']
            if clientid in keys:
                ...
                with open(source_filename, 'r') as input_file:
                    r_decrypt = self.api('/transit/decrypt/{}'.format(clientid),
    ...
    def encrypt(self, clientid, encoded_data, dest_filename):
        r = self.api('/transit/keys', 'LIST')
        if r.status_code == 200:
            keys = r.json()['data']['keys']
            if clientid in keys:
                self.logger.info('Encrypting Data')
                r_encrypt = self.api('/transit/encrypt/{}'.format(clientid),
